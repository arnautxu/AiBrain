...
- La memoria conserva las escrituras manuales y añade extracción automática después del terminal de cada turno: solo reconoce preferencias, hechos estables y decisiones, descarta credenciales/instrucciones efímeras, deduplica por tema, incrementa revisión al corregir y respeta tombstones tras borrar. El store y el runtime siguen privados por usuario/CODEX_HOME; las decisiones de proyecto quedan ligadas al proyecto y nunca se promueven automáticamente a empresa.
- El snapshot acotado prioriza memorias por relevancia respecto al mensaje siguiente y las trata como datos no confiables; cada turno audita IDs y fingerprint sin copiar contenido.
...
- Los browser tools expuestos a Codex forman un namespace cerrado (`open`, `read`, `screenshot`, `scroll`, `click`, `type`, `tabs`, `downloads`); se ejecutan sin pausa interactiva y los reintentos se deduplican por `toolCallId` con permiso, target evidence y binding usuario/thread/turn/item.
...
- Memoria manual y automática: pruebas HTTP+service para auth, aislamiento, create/replay/list/revoke, extracción background, deduplicación, filtro de secretos, revisión/corrección, borrado y recuperación relevante.
