...
- `AIBRAIN_ADMIN_USER_IDS` inicializa propietarios/administradores en el primer arranque. Si no está definido, se reutiliza `AIBRAIN_USAGE_ADMIN_USER_IDS` para mantener compatibilidad.
...
Roles y grupos se guardan en `dataRoot/workspace-admin/state.json`. La política efectiva es la intersección del rol y todos los grupos: cualquier `false` bloquea. Los switches de apps se aplican en Settings, chat y navegador; los bloqueos de capacidades se incorporan al snapshot inmutable de permisos del turno.
