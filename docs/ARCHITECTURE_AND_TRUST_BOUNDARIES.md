...
| Contexto interno de producto no citable | implemented; validated locally | [loader](../src/runtime/internal-agent-context.ts), [brief Arnall](../config/internal-agent-context/arnall.md), [tests](../src/runtime/internal-agent-context.test.ts); el sandbox enmascara el directorio server-only |
| Documentos empresa/departamento/proyecto/privado | implemented; validated locally | [red](../src/documents/enterprise-document-network.ts), [herramientas](../src/runtime/enterprise-documents/dynamic-tools.ts), [tests](../src/documents/enterprise-document-network.test.ts); roots ligados al turno, sin paths host |
