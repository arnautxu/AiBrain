...
/** El text de l'avís quan es vol canviar una casella demanada. */
export function avisDeCanvi(motiu, nom, idioma = 'ca') {
  const qui = nom || (idioma === 'es' ? 'esta persona' : 'aquesta persona');
  const textos = {
    ca: {
      NO_DISPONIBLE: `${qui} va dir que aquell dia no podia treballar. Segur que li vols posar torn?`,
      TORN_DEMANAT: `${qui} va demanar expressament aquell torn. Segur que el vols canviar?`,
      FESTA_CANVIADA: `${qui} va demanar aquell dia lliure. Segur que li vols posar torn?`,
      TORN_CANVIAT: `${qui} havia demanat un altre torn aquell dia. Segur que el vols canviar?`,
    },
    es: {
      NO_DISPONIBLE: `${qui} dijo que ese día no podía trabajar. ¿Seguro que le quieres poner turno?`,
      TORN_DEMANAT: `${qui} pidió expresamente ese turno. ¿Seguro que lo quieres cambiar?`,
      FESTA_CANVIADA: `${qui} pidió ese día libre. ¿Seguro que le quieres poner turno?`,
      TORN_CANVIAT: `${qui} había pedido otro turno ese día. ¿Seguro que lo quieres cambiar?`,
    },
  };
  return (textos[idioma] || textos.ca)[motiu] || null;
}
