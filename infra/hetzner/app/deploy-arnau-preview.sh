...
readonly INSTALLATION_ID="arnau-preview"
readonly COMPOSE_PROJECT="aibrain-arnau-preview"
readonly RELEASE_ROOT="/opt/aibrain-arnau-preview"
readonly RELEASES_DIR="${RELEASE_ROOT}/releases"
readonly CONFIG_DIR="/etc/aibrain/arnau-preview"
...
readonly APP_REPOSITORY="${REGISTRY}/aibrain-arnau-preview"
readonly EGRESS_REPOSITORY="${REGISTRY}/aibrain-arnau-preview-egress"
readonly CONTEXT_ROOT="/var/lib/docker/volumes/aibrain-arnau-preview-data/_data/company-context"
readonly PUBLIC_URL="https://arnau.167.233.146.105.sslip.io"
readonly MAX_ARCHIVE_BYTES=$((64 * 1024 * 1024))
readonly MIN_FREE_BYTES=$((12 * 1024 * 1024 * 1024))

fail() {
  printf 'ARNAU_PREVIEW_DEPLOY_FAILED: %s\n' "$1" >&2
  exit 1
}
...
main() {
  ...
  flock --exclusive --nonblock 9 || fail "another Arnau preview deployment is running"
  ...
  if [[ -f "$STATE_FILE" ]] && jq -e --arg revision "$revision" '.current.revision == $revision' "$STATE_FILE" >/dev/null; then
    printf 'ARNAU_PREVIEW_DEPLOY_ALREADY_CURRENT revision=%s\n' "$revision"
    exit 0
  fi
  ...
  [[ -f "${release_dir}/config/installations/arnau.preview.example.json" ]] || fail "release archive has no Arnau preview installation config"
  ...
  install -m 0400 -o root -g root "${release_dir}/config/installations/arnau.preview.example.json" "$target_config"
  ...
  bash -n "${release_dir}/infra/hetzner/app/deploy-arnau-preview.sh"
  install -m 0700 -o root -g root \
    "${release_dir}/infra/hetzner/app/deploy-arnau-preview.sh" \
    /usr/local/sbin/aibrain-arnau-deploy-gateway

  jq -n --arg revision "$revision" --arg image "$app_image" --arg egressImage "$egress_image" \
    '{schemaVersion:1,installationId:"arnau-preview",revision:$revision,image:$image,egressImage:$egressImage,deployedAt:(now|todateiso8601)}' \
  ...
  printf 'ARNAU_PREVIEW_DEPLOY_OK revision=%s\n' "$revision"
}
