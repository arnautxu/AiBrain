    ...
    try:
        if value['operation'] == 'browse':
            ...
            with folder_module().interactive_access(manifest, catalogue=False):
                return {**files.search(manifest, **value['input'], **({'run': channel.browse} if channel else {})), 'sourceChecked': True, 'lookupMode': 'live'}
        ...
        if cached_only:
            return None
        with folder_module().interactive_access(manifest, catalogue=False):
            return files.read(manifest, **value["input"], **({"call": channel.copy} if channel else {}))
        ...
        if code not in {"SERVER_FILES_BUSY", "WINDOWS_PATH_UNAVAILABLE", "SERVER_FORMAT_NOT_READABLE", "SERVER_TEXT_UNAVAILABLE", "SERVER_PART_UNAVAILABLE", "RDP_DRIVE_REDIRECTION_DISABLED", "SERVER_SOURCE_CHANGED", "SERVER_COPY_LIMIT", "SERVER_CHANNEL_QUARANTINED", "SERVER_CHANNEL_POLICY_CHANGED", "SERVER_FILES_TIMEOUT"}:
