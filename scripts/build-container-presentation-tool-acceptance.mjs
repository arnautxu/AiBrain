...
const output = path.join(repositoryRoot, "dist", "container-presentation-tool-acceptance.mjs");
...
await build({
  entryPoints: [path.join(repositoryRoot, "scripts", "container-presentation-tool-acceptance.ts")],
  outfile: output,
  bundle: true,
  platform: "node",
  format: "esm",
...
if (bundle.includes("This module cannot be imported from a Client Component")) {
  throw new Error("container presentation tool acceptance retained the server-only client sentinel");
}
