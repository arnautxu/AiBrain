...
async function readSource(pathname, label) {
    let stat;
    try {
...
async function readJsonSource(pathname, label) {
    const source = await readSource(pathname, label);
    try {
        return { value: JSON.parse(source.text), fingerprint: source.fingerprint };
    }
    catch {
...
function parseReleaseState(value, candidateSha) {
    ...
    if (value.current.revision !== candidateSha)
        throw new Error("Deploy state revision does not match the requested candidate.");
...
async function command(adapter, args, label) {
...
async function inspectContainer(adapter, container, expectedImage, candidateSha) {
    if (!CONTAINER.test(container))
        throw new Error("Container identity must be a Docker container ID.");
...
async function inspectAppRuntimeRevision(adapter, container, candidateSha) {
    ...
    const revisions = Array.isArray(environment)
        ? environment.filter((item) => typeof item === "string" && item.startsWith("AIBRAIN_REVISION="))
        : [];
...
async function inspectImage(adapter, image, candidateSha) {
...
function provenance(kind, fingerprint) {
    return { kind, sha256: fingerprint };
}
function capturedAt(input) {
    const value = input ?? new Date().toISOString();
    if (!timestampPattern.test(value) || !Number.isFinite(Date.parse(value)))
        throw new Error("capturedAt must be an ISO UTC timestamp.");
    return value;
}
export async function collectReleaseReadbacks(input) {
...
async function main() {
    ...
    if (!path.isAbsolute(dockerBin))
        throw new Error("docker-bin must be an absolute command path.");
    const dockerStat = await lstat(dockerBin);
    if (!dockerStat.isFile() || dockerStat.isSymbolicLink() || (dockerStat.mode & 0o022) !== 0)
        throw new Error("docker-bin must be a non-writable regular file.");
