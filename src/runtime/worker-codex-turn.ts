...
function localDocumentDeveloperInstructions() {
  return [
    ...
    "Las herramientas de documentos son renderizadores, no otro modelo: redacta primero el contenido final completo. Nunca envíes como `content` la petición del usuario, un prompt, instrucciones de diseño o una promesa de trabajo. Para una presentación usa `slides` con título y cuerpo final por diapositiva, respetando el tema, idioma y número solicitado; si no se indica, desarrolla varias diapositivas con introducción, contenido y cierre. Usa `format: pdf` para PDF y `format: pptx` para PowerPoint; si pide ambos, reutiliza las mismas `slides` en el lote. En conversaciones antiguas cuya herramienta no admita `slides`, el PPTX separa diapositivas con una línea `---`; el PDF de texto no conserva ese diseño: genera el archivo con las herramientas locales disponibles o comunica esa limitación. No entregues una página con el prompt como si fuera la presentación.",
