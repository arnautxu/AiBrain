    ...
    <aside className="panel-enter fixed inset-y-0 right-0 z-30 flex w-full max-w-[720px] flex-col border-l border-[#deddd9] bg-[#f7f7f5] shadow-[-24px_0_48px_-40px_rgba(0,0,0,.45)] xl:static xl:max-w-[620px] xl:shadow-none">
      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[240px_1fr]">
        ...
        <section className="scrollbar-thin min-h-0 overflow-y-auto p-5">
          {selected ? <><p className="text-[9px] font-medium text-[#8b8780]">{selected.category === "runtime" ? "Comprovació del sistema" : "Projecte"} · no modifica res</p><h3 className="mt-2 text-[22px] font-semibold tracking-[-.035em] text-[#2b2926]">{selected.name}</h3><p className="mt-3 max-w-[52ch] text-[11px] leading-5 text-[#77736d]">{selected.description}</p><button disabled={!projectId || busy} className="mt-5 inline-flex items-center gap-2 rounded-lg bg-[var(--brain-accent)] px-3 py-2 text-[10px] font-semibold text-[var(--brain-contrast)] disabled:opacity-40" onClick={() => void execute()}>{busy ? <SpinnerGap size={13} className="animate-spin" /> : <Play size={12} weight="fill" />}{busy ? "Executant" : "Executa ara"}</button></> : <div className="rounded-xl border border-[#e1dfda] bg-white p-5"><h3 className="text-[12px] font-semibold text-[#393733]">Cap automatització disponible</h3><p className="mt-2 text-[9px] leading-4 text-[#8c8881]">L’administrador encara no t’ha assignat cap procés automatitzat.</p></div>}
          ...
          {runs.length ? <div className="mt-7"><h4 className="text-[9px] font-semibold text-[#827e77]">Resultats d’aquesta sessió</h4><div className="mt-3 space-y-3">{runs.map((run) => <article key={run.id} className="rounded-xl border border-[#dfded9] bg-white p-3"><div className="flex items-center gap-2 text-[9px] font-medium text-[#4d4a46]">{run.status === "completed" ? <CheckCircle size={12} className="text-[#568060]" /> : <WarningCircle size={12} className="text-[#9a5945]" />}{automations.find((item) => item.id === run.automationId)?.name}<span className="ml-auto text-[8px] font-normal text-[#9b9790]">{new Date(run.finishedAt).toLocaleTimeString("ca", { hour: "2-digit", minute: "2-digit" })}</span></div><div className="scrollbar-thin mt-3 max-h-56 overflow-auto whitespace-pre-wrap rounded-lg bg-[#f3f2ef] p-3 text-[9px] leading-4 text-[#55514b]">{run.output}</div></article>)}</div></div> : selected ? <div className="mt-10 border-t border-[#e1dfda] pt-5 text-[9px] leading-4 text-[#99958e]">Quan executis una automatització, el resultat apareixerà aquí amb un resum entenedor.</div> : null}
        </section>
      </div>
    </aside>
  );
