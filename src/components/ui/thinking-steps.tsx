...
const TriggerRow = forwardRef<HTMLButtonElement, TriggerRowProps>(
  ({ open, indicator, labelClassName, children, className, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        >
          {indicator ? <span className="inline-flex shrink-0 items-center justify-center">{indicator}</span> : null}
          {/* Label with dual-layer text (invisible bold layer reserves width) */}
          <span className={cn("inline-grid min-w-0 max-w-[min(70vw,34rem)] text-left", sizeClasses.text)}>
            <span
              className="col-start-1 row-start-1 truncate invisible"
          ...
          <div className={cn("flex gap-2.5 px-2 py-1.5", shape.item)}>
            ...
            <div className="flex-1 flex flex-col gap-1 min-w-0">
              <span
                className={cn(
                  sizeClasses.text,
                  "leading-tight text-foreground [overflow-wrap:anywhere]",
                  isActive && "thinking-steps-shimmer"
                )}
              ...
              {description && (
                <span className={cn(sizeClasses.text, "text-muted-foreground leading-snug [overflow-wrap:anywhere]")}>
                  {description}
                </span>
              )}
              {children}
