...
export function LandingTasks({ tasks, variant, disabled, onSelect }: {
  ...
  return <div ref={rootRef} className={`landing-tasks landing-tasks-${variant}`}>
    ...
    {open ? <ConnectorPopover anchor={openerRef} triggerAligned><div ref={menuRef} id={id} role="menu" aria-label={schedule ? t("Horarios del equipo") : t("Tareas recurrentes")} className="max-h-[inherit] overflow-y-auto overscroll-contain" onKeyDown={(event) => { event.stopPropagation(); onKeyDown(event); }}>
