...
export function ServerPicker({ projectId, selected, onSelect, onClose, returnFocus }: {
  ...
  useEffect(() => {
    ...
    fetch(`/api/server-files?${new URLSearchParams({ projectId, query })}`, { cache: "no-store", signal: controller.signal })
      .then(async response => {
        ...
        if (!response.ok || !data.available || (!navigation && data.sourceChecked !== true)) throw new Error(data.warning || "No se ha podido consultar Windows.");
  ...
  return createPortal(<div data-testid="server-backdrop" className="fixed inset-0 z-[90] flex items-center justify-center bg-black/35 p-3" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={focus} role="dialog" aria-modal="true" aria-label="Server" aria-describedby="server-description" tabIndex={-1} className="flex max-h-[85dvh] w-full max-w-3xl flex-col rounded-2xl border border-[var(--border)] bg-[var(--surface-raised)] p-4 text-[var(--text)] shadow-xl">
      ...
      <p id="server-description" className="mb-3 text-xs text-[var(--text-muted)]">Archivos del servidor Windows. Selecciona hasta 5 referencias para trabajar en el chat. Los originales son de sólo lectura.</p>
      <div className="mb-3 flex items-center gap-2">
        <button type="button" className="touch-target rounded-lg border border-[var(--border)] px-3 py-2 text-sm" disabled={busy || location === "home"} onClick={() => location === "server:/" ? navigate("home") : navigate(location.replace(/\/$/, "").split("/").slice(0, -1).join("/").replace(/^server:$/, "server:/"))}>Subir</button>
        <span className="min-w-0 flex-1 truncate text-sm" title={location}>{location === "home" ? "Carpetas de trabajo" : location === "server:/" ? "Unidades" : location.replace("server:/", "")}</span>
        <button type="button" aria-label={location === "home" ? "Actualizar carpetas de trabajo" : "Actualizar desde Windows"} disabled={busy} className="touch-target rounded-lg border border-[var(--border)] p-2" onClick={() => { navigate(location); }}><ArrowClockwise size={19}/></button>
      </div>
      {location === "home" ? <button type="button" className="mb-3 w-fit rounded-lg border border-[var(--border)] px-3 py-2 text-sm" onClick={() => navigate("server:/")}>Explorar unidades</button> : <button type="button" className="mb-3 w-fit rounded-lg px-2 py-1 text-sm text-[var(--text-muted)]" onClick={() => navigate("home")}>Carpetas de trabajo</button>}
      <div className="min-h-40 overflow-y-auto" aria-busy={busy}>
        {busy ? <p role="status" className="p-5 text-sm">{location === "home" ? "Cargando carpetas de trabajo…" : "Consultando Windows… Puedes cerrar esta ventana mientras se completa."}</p> : null}
        ...
        {page ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">{page.results.map(item => <div key={item.path} className="min-w-0 rounded-xl border border-[var(--border)] p-3">
          <button type="button" className="flex w-full flex-col items-center gap-2 rounded-lg py-3 text-sm hover:bg-[var(--surface-hover)]" aria-label={item.kind === "directory" ? `Abrir ${item.name}` : `Seleccionar ${item.name}`} onClick={() => item.kind === "directory" ? navigate(serverDirectoryQuery(item.path)) : toggle(item)}>
            ...
            <span className="w-full truncate" title={item.name}>{!page.navigation && isDrive(item) ? `Unidad ${item.name}:` : item.name}</span>
          </button>
          {!page.navigation && !isDrive(item) ? <label className="flex cursor-pointer items-center gap-2 text-xs"><input type="checkbox" aria-label={`Adjuntar ${item.name}`} checked={selection.some(ref => ref.path === item.path)} disabled={selection.length >= 5 && !selection.some(ref => ref.path === item.path)} onChange={() => toggle(item)}/>Seleccionar</label> : null}
        </div>)}</div> : null}
        {page && !page.results.length ? <p className="p-4 text-sm">{page.navigation ? "Explora las unidades para ver las carpetas disponibles." : "Sin elementos visibles en esta ubicación."}</p> : null}
      </div>
      {page ? <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-[var(--text-muted)]"><span>{page.navigation ? "Abre una carpeta para consultar sus archivos." : <>Consultado: {new Date(page.checkedAt!).toLocaleTimeString()}{page.limited ? " · listado parcial" : ""}</>}</span>{page.nextQuery ? <button type="button" className="touch-target rounded border px-3 py-2" onClick={() => navigate(page.nextQuery!)}>Siguiente página</button> : null}</div> : null}
