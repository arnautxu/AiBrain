...
export function ServerPicker({ projectId, selected, onSelect, onClose, returnFocus }: {
  ...
  useEffect(() => {
    ...
    fetch(`/api/server-files?${new URLSearchParams({ projectId, query })}`, { cache: "no-store", signal: controller.signal })
      .then(async response => {
        ...
        if (!response.ok || !data.available || data.sourceChecked !== true) throw new Error(data.warning || t("No se ha podido consultar Windows."));
  ...
  return createPortal(<div data-testid="server-backdrop" className="fixed inset-0 z-[90] flex items-center justify-center bg-black/35 p-3" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={focus} role="dialog" aria-modal="true" aria-label={t("Server")} aria-describedby="server-description" tabIndex={-1} className="flex max-h-[85dvh] w-full max-w-3xl flex-col rounded-2xl border border-[var(--border)] bg-[var(--surface-raised)] p-4 text-[var(--text)] shadow-xl">
      <div className="flex items-center justify-between gap-2"><h2 className="font-semibold">{t("Server")}</h2><button type="button" aria-label={t("Cerrar Server")} className="touch-target rounded p-2" onClick={onClose}><X size={20}/></button></div>
      ...
      <div className="mb-3 flex items-center gap-2">
        <button type="button" className="touch-target rounded-lg border border-[var(--border)] px-3 py-2 text-sm" disabled={busy || location === "server:/"} onClick={() => navigate(location.replace(/\/$/, "").split("/").slice(0, -1).join("/").replace(/^server:$/, "server:/"))}>{t("Subir")}</button>
        <span className="min-w-0 flex-1 truncate text-sm" title={location}>{location === "server:/" ? t("Unidades") : location.replace("server:/", "")}</span>
        <button type="button" aria-label={t("Actualizar desde Windows")} disabled={busy} className="touch-target rounded-lg border border-[var(--border)] p-2" onClick={() => { navigate(location); }}><ArrowClockwise size={19}/></button>
      </div>
      <div className="min-h-40 overflow-y-auto" aria-busy={busy}>
        {busy ? <p role="status" className="p-5 text-sm">{t("Consultando Windows… Puedes cerrar esta ventana mientras se completa.")}</p> : null}
        {error ? <p role="alert" className="p-4 text-sm">{error} {" "}{t("Usa Actualizar para reintentarlo. No se muestra una copia antigua.")}</p> : null}
        {page ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">{page.results.map(item => <div key={item.path} className="min-w-0 rounded-xl border border-[var(--border)] p-3">
          <button type="button" className="flex w-full flex-col items-center gap-2 rounded-lg py-3 text-sm hover:bg-[var(--surface-hover)]" aria-label={item.kind === "directory" ? t("Abrir {p0}", { p0: item.name }) : `Seleccionar ${item.name}`} onClick={() => item.kind === "directory" ? navigate(serverDirectoryQuery(item.path)) : toggle(item)}>
            {isDrive(item) ? <HardDrive size={38} aria-hidden="true"/> : item.kind === "directory" ? <Folder size={38} weight="fill" className="text-sky-500"/> : <File size={34}/>}
            <span className="w-full truncate" title={item.name}>{isDrive(item) ? `Unidad ${item.name}:` : item.name}</span>
          </button>
          {!isDrive(item) ? <label className="flex cursor-pointer items-center gap-2 text-xs"><input type="checkbox" aria-label={`Adjuntar ${item.name}`} checked={selection.some(ref => ref.path === item.path)} disabled={selection.length >= 5 && !selection.some(ref => ref.path === item.path)} onChange={() => toggle(item)}/>{t("Seleccionar")}</label> : null}
        </div>)}</div> : null}
        {page && !page.results.length ? <p className="p-4 text-sm">{t("Sin elementos visibles en esta ubicación.")}</p> : null}
      </div>
      {page ? <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-[var(--text-muted)]"><span>{t("Consultado:")}{" "}{new Date(page.checkedAt).toLocaleTimeString()}{page.limited ? " · listado parcial" : ""}</span>{page.nextQuery ? <button type="button" className="touch-target rounded border px-3 py-2" onClick={() => navigate(page.nextQuery!)}>{t("Siguiente página")}</button> : null}</div> : null}
