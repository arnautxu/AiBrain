    ...
    return (
      <main className="grid min-h-[100dvh] place-items-center bg-[#f1f2ef] px-5 py-10 text-[#292825]">
        <section className="w-full max-w-lg rounded-3xl border border-[#deddd8] bg-[#fbfbfa] p-7 shadow-[0_24px_70px_-48px_rgba(35,32,27,.55)] md:p-10">
          ...
          <h1 className="mt-8 text-[30px] font-semibold leading-[1.05] tracking-[-.045em]">El teu rol encara no està definit.</h1>
          ...
          <form action="/api/auth/logout" method="post" className="mt-8">
            <button className="rounded-xl border border-[#d7d5d0] bg-white px-4 py-2.5 text-[13px] font-semibold text-[#4b4843]">Tanca la sessió</button>
          </form>
        </section>
      </main>
    );
      ...
      <div className="mx-auto grid min-h-[calc(100dvh-4.5rem)] max-w-[1120px] overflow-hidden rounded-[28px] border border-[#dcdad5] bg-[#fbfbfa] shadow-[0_30px_90px_-58px_rgba(35,32,27,.6)] lg:grid-cols-[340px_1fr]">
        <aside className="flex flex-col bg-[#222320] p-5 text-white md:p-8">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-[13px] font-semibold">{assistantName}</p><p className="mt-0.5 text-[12px] text-white/55">{tenantName}</p></div>
          </div>
          <div className="mt-8 lg:mt-14">
            <h1 className="text-[25px] font-semibold leading-[1.04] tracking-[-.045em] lg:text-[30px]">Benvingut, {memberName}. Tot preparat per començar a treballar.</h1>
            <p className="mt-4 hidden text-[13px] leading-6 text-white/60 sm:block">L’empresa ja ha configurat el context, les eines i els límits. Tu només confirmes com vols treballar.</p>
          </div>
          <ol className="mt-7 grid grid-cols-3 gap-2 lg:mt-12 lg:block lg:space-y-3">
            {steps.map((item, index) => {
              ...
              return (
                <li key={item.label} className={`flex flex-col items-center gap-1.5 rounded-xl px-2 py-2.5 text-[12px] transition lg:flex-row lg:gap-3 lg:px-3 lg:py-3 ${active ? "bg-white/10 text-white" : "text-white/50"}`}>
                  ...
                  <span className="text-[12px] lg:ml-auto">0{index + 1}</span>
                </li>
              );
            })}
          </ol>
          <div className="mt-auto hidden items-center gap-2 border-t border-white/10 pt-5 text-[12px] leading-5 text-white/50 lg:flex"><LockKey size={13} /> Els permisos només els pot canviar un admin.</div>
        </aside>
        ...
        <section className="flex min-h-[620px] flex-col p-6 md:p-10 lg:p-14">
          <div className="mb-10 flex items-center justify-between">
            ...
            <span className="text-[12px] font-medium text-[#88847d]">{step + 1} de {steps.length}</span>
          </div>

          {step === 0 ? (
            <div className="panel-enter max-w-2xl">
              <h2 className="text-[34px] font-semibold tracking-[-.045em]">{assignment.jobTitle}</h2>
              ...
              <div className="mt-8 rounded-2xl border border-[#dfddd8] bg-white p-5 md:p-6">
                <h3 className="text-[13px] font-semibold">Les teves responsabilitats</h3>
              ...
              <label className="mt-6 block">
                <span className="text-[12px] font-semibold text-[#77736d]">Alguna cosa no encaixa? <span className="font-normal text-[#918d86]">Opcional · no canvia permisos automàticament</span></span>
          ...
          {step === 1 ? (
            <div className="panel-enter max-w-2xl">
              <p className="text-[13px] font-medium text-[#77736d]">La teva manera de treballar</p>
              <h2 className="mt-3 text-[34px] font-semibold tracking-[-.045em]">Personalitza només el que és teu.</h2>
              <div className="mt-8 grid gap-6">
                <fieldset>
                  <legend className="text-[12px] font-semibold text-[#77736d]">Idioma habitual</legend>
                  <div className="mt-3 flex flex-wrap gap-2">{languageOptions.map((option) => <button type="button" key={option.value} aria-pressed={language === option.value} onClick={() => setLanguage(option.value)} className={`rounded-xl border px-4 py-3 text-[13px] font-semibold transition ${language === option.value ? "border-[#292825] bg-[#292825] text-white" : "border-[#d9d7d2] bg-white text-[#5f5b56] hover:border-[#aaa69f]"}`}>{option.label}</button>)}</div>
                </fieldset>
                <fieldset>
                  <legend className="text-[12px] font-semibold text-[#77736d]">Nivell de detall</legend>
                  <div className="mt-3 grid gap-2 sm:grid-cols-3">{responseOptions.map((option) => <button type="button" key={option.value} aria-pressed={responseStyle === option.value} onClick={() => setResponseStyle(option.value)} className={`rounded-xl border p-4 text-left transition ${responseStyle === option.value ? "border-[#292825] bg-[#f1f1ee]" : "border-[#d9d7d2] bg-white hover:border-[#aaa69f]"}`}><span className="block text-[13px] font-semibold">{option.label}</span><span className="mt-1.5 block text-[12px] leading-5 text-[#77736d]">{option.detail}</span></button>)}</div>
                </fieldset>
              </div>
              <div className="mt-8 rounded-2xl bg-[#eeefec] p-5">
                <div className="flex items-center gap-2"><LockKey size={14} /><h3 className="text-[13px] font-semibold">Configurat per {tenantName}</h3></div>
                <div className="mt-4 flex flex-wrap gap-2">{capabilities.map((capability) => <span key={capability} className="rounded-lg border border-[#d8d9d5] bg-white/70 px-2.5 py-1.5 text-[12px] text-[#69665f]">{capability}</span>)}</div>
                <p className="mt-4 text-[12px] leading-5 text-[#817e77]">Pots utilitzar aquestes funcions, però no canviar-ne els permisos ni les polítiques.</p>
              </div>
            </div>
          ) : null}
          ...
          {step === 2 ? (
            <div className="panel-enter max-w-2xl">
              <p className="text-[13px] font-medium text-[#77736d]">El teu primer resultat</p>
              ...
              <div className="mt-8 rounded-2xl border border-[#d9d7d2] bg-white p-6 md:p-8">
                ...
                <div className="mt-6 flex items-center gap-2 text-[12px] text-[#77736d]"><CheckCircle size={13} /> S’obrirà preparada al workbench; tu decidiràs quan enviar-la.</div>
              </div>
              {error ? <p role="alert" className="mt-5 rounded-xl bg-[#fff0ea] px-4 py-3 text-[12px] text-[#8b4f3c]">{error}</p> : null}
            </div>
          ) : null}

          <div className="mt-auto flex items-center justify-between border-t border-[#e5e3df] pt-6">
            <button type="button" disabled={step === 0 || submitting} onClick={() => setStep((current) => Math.max(0, current - 1))} className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-[13px] font-semibold text-[#77736d] hover:bg-[#f0efec] disabled:invisible"><ArrowLeft size={13} /> Enrere</button>
            {step < steps.length - 1 ? (
              <button type="button" onClick={() => setStep((current) => Math.min(steps.length - 1, current + 1))} className="flex items-center gap-2 rounded-xl bg-[#292825] px-5 py-3 text-[13px] font-semibold text-white">Continua <ArrowRight size={13} /></button>
            ) : (
              <button type="button" disabled={submitting} onClick={() => void complete()} className="flex items-center gap-2 rounded-xl bg-[#292825] px-5 py-3 text-[13px] font-semibold text-white disabled:opacity-50">{submitting ? "Preparant…" : "Comença la missió"}<RocketLaunch size={13} /></button>
            )}
          </div>
        </section>
      </div>
