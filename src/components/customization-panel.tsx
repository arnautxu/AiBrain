...
import { isEmployeeWeeklyBudgetResponse, type EmployeeWeeklyBudgetResponse } from "@/usage/employee-budget-contract";
...
export function CustomizationPanel({ productName, open, initialTab = "appearance", runtimeStatus, projects = [], threads = [], archiveBusy = false, onRestoreProject, onRestoreThread, onSettingsSnapshot, activeProjectId = null, returnFocusRef, onClose }: Props) {
  ...
  const [employeeBudget, setEmployeeBudget] = useState<EmployeeWeeklyBudgetResponse | null>(null);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      fetch("/api/usage/me", { cache: "no-store" }).then(async (response) => response.ok ? response.json() as Promise<PersonalUsageResponse | EmployeeWeeklyBudgetResponse> : null),
    ]).then(async ([snapshot, personal]) => {
      if (cancelled) return;
      if (!isSettingsSnapshot(snapshot)) { setError(t("No se ha podido cargar la configuración.")); return; }
      setSettings(snapshot); onSettingsSnapshot?.(snapshot);
      if (isEmployeeWeeklyBudgetResponse(personal)) {
        setEmployeeBudget(personal); setPersonalUsage(null); setCompanyUsage(null);
        return;
      }
      setEmployeeBudget(null); setPersonalUsage(personal);
      if (!snapshot.company.isAdmin) return;
      const response = await fetch("/api/usage/company", { cache: "no-store" });
      const company = response.ok ? await response.json() as CompanyUsageResponse | EmployeeWeeklyBudgetResponse : null;
      if (!cancelled) {
        if (isEmployeeWeeklyBudgetResponse(company)) {
          setEmployeeBudget(company); setPersonalUsage(null); setCompanyUsage(null);
        } else setCompanyUsage(company);
      }
    }).catch(() => { if (!cancelled) setError(t("No se ha podido cargar la configuración.")); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [loadAttempt, onSettingsSnapshot, open, t]);
  ...
  const save = async (patch: SettingsPatch, key: string) => {
    ...
    try {
      ...
      if (!response.ok || !isSettingsSnapshot(result)) {
        const message = result && typeof result === "object" && "error" in result && typeof result.error === "string" ? result.error : t("No se ha podido guardar el cambio.");
        throw new Error(message);
      }
      setSettings(result); onSettingsSnapshot?.(result);
    } catch (cause) { setError(cause instanceof Error ? cause.message : t("No se ha podido guardar el cambio.")); }
  ...
  const tabs: Array<{ id: Tab; label: string; icon: ReactNode }> = [
    { id: "appearance", label: t("Apariencia"), icon: <PaintBrush size={17} /> },
    { id: "connectors", label: t("Conectores"), icon: <Plugs size={17} /> },
    { id: "memory", label: t("Memoria"), icon: <Brain size={17} /> },
    { id: "notifications", label: t("Avisos"), icon: <Bell size={17} /> },
    { id: "archived", label: t("Archivados"), icon: <Archive size={17} /> },
    { id: "usage", label: t("Uso"), icon: <Gauge size={17} /> },
  ];
  return <AnimatePresence initial={false}>{open ? (
    <OverlayPresenceLayer key="customization-panel" origin="center" className="workspace-overlay fixed inset-0 z-50 grid place-items-center p-0 md:p-6">
      {(surfaceMotion) => <>
        <button className="absolute inset-0" aria-label={t("Cerrar configuración")} onClick={onClose} />
        <motion.section {...surfaceMotion} ref={panelRef} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t("Configuración de {p0}", { p0: productName })} className="workspace-panel relative flex h-full w-full max-w-[840px] flex-col overflow-hidden border border-[var(--border)] bg-[var(--surface-raised)] shadow-[var(--shadow-lg)] md:h-[min(680px,calc(100dvh-3rem))] md:rounded-[22px]">
          <header className="workspace-panel-header flex shrink-0 items-center justify-between border-b border-[var(--border-subtle)] px-5"><div><h2 className="workspace-panel-title">{t("Configuración")}</h2><p className="workspace-panel-subtitle mt-0.5 hidden sm:block">{t("Tu apariencia, avisos, archivados y uso.")}</p></div><button type="button" aria-label={t("Cerrar")} className="grid size-10 place-items-center rounded-full text-[var(--text-subtle)] hover:bg-[var(--surface-hover)]" onClick={onClose}><X size={17} /></button></header>
          <div className="flex min-h-0 flex-1 flex-col md:flex-row"><nav aria-label={t("Secciones de configuración")} className="flex shrink-0 gap-1 overflow-x-auto border-b border-[var(--border-subtle)] p-2 md:w-52 md:flex-col md:border-b-0 md:border-r md:p-3">{tabs.map((item) => <button key={item.id} type="button" aria-current={tab === item.id ? "page" : undefined} className={`flex min-h-10 shrink-0 items-center gap-2.5 rounded-[12px] px-3 text-left text-[12px] font-medium ${tab === item.id ? "bg-[var(--surface-selected)] text-[var(--text)]" : "text-[var(--text-secondary)] hover:bg-[var(--surface-hover)]"}`} onClick={() => setTab(item.id)}>{item.icon}{item.label}</button>)}</nav>
            <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-5 py-6 md:px-8">{error ? <div className="mb-5 flex items-center justify-between gap-3 rounded-[12px] border border-[var(--danger)] bg-[var(--danger-soft)] px-3 py-2 text-[11px] text-[var(--danger)]"><p role="alert">{t(error)}</p>{!settings ? <button type="button" className="shrink-0 rounded-lg border border-current px-3 py-2 font-semibold hover:bg-[var(--surface-hover)]" onClick={retryLoad}>{t("Reintentar")}</button> : null}</div> : null}{loading && !settings ? <p className="text-[12px] text-[var(--text-subtle)]">{t("Cargando configuración…")}</p> : null}{tab === "appearance" ? <Appearance /> : null}{tab === "connectors" ? <Connectors settings={settings} onChanged={retryLoad} /> : null}{tab === "memory" ? <MemorySettings productName={productName} settings={settings} projectId={activeProjectId} /> : null}{tab === "notifications" ? <Notifications settings={settings} busy={savingKey === "notifications"} onSave={save} /> : null}{tab === "archived" ? <Archived projects={projects} threads={threads} busy={archiveBusy} onRestoreProject={onRestoreProject} onRestoreThread={onRestoreThread} /> : null}{tab === "usage" ? <Usage usage={usage} companyUsage={companyUsage} usedPercent={usedPercent} runtimeStatus={runtimeStatus} employeeBudget={employeeBudget} /> : null}</div>
          </div>
        </motion.section>
      </>}
...
function Connectors({ settings, onChanged }: { settings: SettingsSnapshot | null; onChanged: () => void }) {
  ...
  const matches = settings?.connectors.filter(c => `${c.label} ${connectorPresentation(c.id).description}`.toLocaleLowerCase("es").includes(query.toLocaleLowerCase("es"))) ?? [];
  const connected = matches.filter(c => c.status === "connected");
  const available = matches.filter(c => c.status !== "connected");
  const row = (connector: NonNullable<SettingsSnapshot>["connectors"][number]) => <article key={connector.id} className="flex min-h-20 items-center gap-3 border-b border-[var(--border-subtle)] px-3 py-3 last:border-0">
  ...
  return <section><SectionTitle>{t("Conectores personales")}</SectionTitle>
    ...
    <p className="mt-4 text-[11px] leading-4 text-[var(--text-subtle)]">{t("Catálogo autorizado para tu empresa y tus grupos. Acceso limitado a las herramientas de lectura revisadas.")}</p>
    {error ? <p role="alert" className="mt-3 text-[11px] text-[var(--danger)]">{t(error)}</p> : null}
  </section>;
}
function MemorySettings({ productName, settings, projectId }: { productName: string; settings: SettingsSnapshot | null; projectId: string | null }) {
  const t = useUiText(); return <section><SectionTitle>{t("Memoria de trabajo")}</SectionTitle><div className="mb-5 rounded-[16px] border border-[var(--border)] bg-[var(--surface)] p-4"><p className="text-[12px] font-semibold">{t("Automática y activa en tu entorno privado")}</p><p className="mt-2 text-[11px] leading-5 text-[var(--text-subtle)]">{t("Al terminar un turno,")}{" "}{productName} {" "}{t("guarda en segundo plano solo preferencias, hechos estables y decisiones útiles. No añade espera a la respuesta.")}</p><ul className="mt-3 space-y-1 text-[10px] text-[var(--text-subtle)]"><li>{t("Privacidad: almacenamiento y runtime separados por empleado.")}</li><li>{t("Seguridad: secretos, credenciales e instrucciones efímeras se descartan.")}</li><li>{t("Control: cada recuerdo conserva procedencia y revisión, y se puede corregir o eliminar.")}</li></ul></div>{settings?.memory.enabled ? <MemoryPanel open embedded productName={productName} projectId={projectId} onClose={() => undefined} /> : null}</section>; }
function Notifications({ settings, busy, onSave }: { settings: SettingsSnapshot | null; busy: boolean; onSave: (patch: SettingsPatch, key: string) => Promise<void> }) {
  const t = useUiText(); return <section><SectionTitle>{t("Avisos de trabajo")}</SectionTitle><p className="mb-4 text-[12px] leading-5 text-[var(--text-muted)]">{t("Controla los avisos de esta aplicación. No se solicita permiso del sistema operativo desde esta pantalla.")}</p><div className="divide-y divide-[var(--border-subtle)] border-y border-[var(--border-subtle)]"><NotificationToggle field="backgroundTurns" title={t("Tareas terminadas")} description={t("Avisa cuando una conversación termina mientras trabajas en otra.")} settings={settings} busy={busy} onSave={onSave} /><NotificationToggle field="approvals" title={t("Aprobaciones pendientes")} description={t("Avisa cuando una acción necesita tu decisión.")} settings={settings} busy={busy} onSave={onSave} /><NotificationToggle field="failures" title={t("Errores y bloqueos")} description={t("Avisa cuando una tarea requiere atención.")} settings={settings} busy={busy} onSave={onSave} /><NotificationToggle field="sound" title={t("Sonido")} description={t("Reproduce un sonido discreto para avisos dentro de la aplicación.")} settings={settings} busy={busy} onSave={onSave} /></div></section>; }
function Archived({ projects, threads, busy, onRestoreProject, onRestoreThread }: { projects: WorkbenchProject[]; threads: WorkbenchThread[]; busy: boolean; onRestoreProject?: (project: WorkbenchProject) => void; onRestoreThread?: (thread: WorkbenchThread) => void }) {
  ...
  if (!archivedProjects.length && !archivedThreads.length) return <section><SectionTitle>{t("Archivados")}</SectionTitle><div className="rounded-[16px] border border-dashed border-[var(--border)] px-5 py-8 text-center"><p className="text-[12px] font-semibold text-[var(--text)]">{t("No hay elementos archivados")}</p><p className="mt-1 text-[11px] text-[var(--text-subtle)]">{t("Las conversaciones y proyectos archivados aparecerán únicamente aquí.")}</p></div></section>;
  return <section><SectionTitle>{t("Archivados")}</SectionTitle><p className="mb-5 text-[12px] leading-5 text-[var(--text-muted)]">{t("Restaura desde aquí las conversaciones y proyectos que quieras devolver al sidebar.")}</p>{archivedProjects.length ? <div className="mb-6"><h4 className="mb-2 text-[11px] font-semibold text-[var(--text-secondary)]">{t("Proyectos")}</h4><div className="divide-y divide-[var(--border-subtle)] rounded-[14px] border border-[var(--border)]">{archivedProjects.map((project) => <div key={project.id} className="flex items-center gap-3 px-4 py-3"><Archive size={15} className="shrink-0 text-[var(--text-subtle)]" /><span className="min-w-0 flex-1 truncate text-[12px] font-medium">{project.name}</span><button type="button" disabled={busy || !onRestoreProject} className="rounded-lg border border-[var(--border)] px-3 py-2 text-[11px] font-semibold text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] disabled:opacity-45" onClick={() => onRestoreProject?.(project)}>{t("Restaurar")}</button></div>)}</div></div> : null}{archivedThreads.length ? <div><h4 className="mb-2 text-[11px] font-semibold text-[var(--text-secondary)]">{t("Conversaciones")}</h4><div className="divide-y divide-[var(--border-subtle)] rounded-[14px] border border-[var(--border)]">{archivedThreads.map((thread) => { const parent = projects.find((project) => project.id === thread.projectId); const parentArchived = archivedProjectIds.has(thread.projectId); return <div key={thread.id} className="flex items-center gap-3 px-4 py-3"><Archive size={15} className="shrink-0 text-[var(--text-subtle)]" /><span className="min-w-0 flex-1"><span className="block truncate text-[12px] font-medium">{thread.title}</span>{parent ? <span className="mt-0.5 block truncate text-[10px] text-[var(--text-subtle)]">{parent.name}</span> : null}</span><button type="button" disabled={busy || parentArchived || !onRestoreThread} title={parentArchived ? t("Restaura primero el proyecto") : undefined} className="rounded-lg border border-[var(--border)] px-3 py-2 text-[11px] font-semibold text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] disabled:opacity-45" onClick={() => onRestoreThread?.(thread)}>{t("Restaurar")}</button></div>; })}</div></div> : null}</section>;
}
...
function Usage({ usage, companyUsage, usedPercent, employeeBudget }: { usage: PersonalUsageResponse | CompanyUsageResponse | null; companyUsage: CompanyUsageResponse | null; usedPercent: number | null; runtimeStatus: RuntimeStatus; employeeBudget: EmployeeWeeklyBudgetResponse | null }) {
  const locale = useUiLocale();
  const t = useUiText();
  if (employeeBudget) {
    const budget = employeeBudget.budget;
    return <section><SectionTitle>{t("Saldo semanal compartido")}</SectionTitle>
      {budget?.initialized ? <div className="rounded-[18px] border border-[var(--border)] bg-[var(--surface)] p-5">
        <p className="text-[28px] font-semibold">{t("Saldo semanal disponible: {percent}%", { percent: budget.remainingPercent! })}</p>
        <p className="mt-3 text-[12px] text-[var(--text-secondary)]">{t("Se renueva el {date} (hora de Madrid).", { date: new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short", timeZone: "Europe/Madrid" }).format(new Date(budget.resetAt)) })}</p>
        {budget.threshold === 100 ? <p role="alert" className="mt-3 text-[12px]">{t("Límite semanal agotado. Las nuevas solicitudes están bloqueadas hasta la renovación.")}</p> : null}
      </div> : <p role="status" className="text-[12px]">{t("No se puede comprobar el saldo semanal. No se pueden iniciar nuevas solicitudes.")}</p>}
    </section>;
  }
  const internal = usage?.internal; const minutes = internal ? Math.round(internal.totalDurationMs / 60_000) : null; return <section><SectionTitle>{t("Suscripción compartida")}</SectionTitle><div className="rounded-[18px] border border-[var(--border)] bg-[var(--surface)] p-5"><div className="flex items-end justify-between gap-4"><div><p className="text-[12px] font-semibold">{t("Servicio de empresa")}</p><p className="mt-1 text-[11px] text-[var(--text-subtle)]">{t("Consumo global del servicio compartido")}</p></div><p className="text-[28px] font-semibold tracking-[-.03em]">{usedPercent === null ? "—" : `${Math.round(usedPercent)}%`}</p></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-[var(--surface-muted)]"><div className="h-full rounded-full bg-[var(--brain-accent)]" style={{ width: `${usedPercent ?? 0}%` }} /></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><Metric label={t("Turnos")} value={number(internal?.turns ?? 0)} /><Metric label={t("Minutos trabajados")} value={minutes === null ? "—" : `${number(minutes)} min`} /></div><p className="mt-3 text-[10px] leading-4 text-[var(--text-subtle)]">{t("Suma de la duración de los turnos finalizados, redondeada al minuto más cercano.")}</p>{companyUsage ? <div className="mt-7"><SectionTitle>{t("Uso por empleado")}</SectionTitle><div className="divide-y divide-[var(--border-subtle)] rounded-[16px] border border-[var(--border)]">{companyUsage.members.map((member) => <div key={member.userId} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 px-4 py-3 text-[11px]"><span className="truncate font-semibold">{member.displayName}</span><span className="text-[var(--text-subtle)]">{number(member.usage.turns)} {" "}{t("turnos")}</span><span className="min-w-20 text-right text-[var(--text-secondary)]">{number(Math.round(member.usage.totalDurationMs / 60_000))} {" "}{t("min")}</span></div>)}</div></div> : null}</section>; }
