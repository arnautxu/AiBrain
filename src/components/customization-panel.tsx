...
import { Bell, Brain, Gauge, PaintBrush, Plugs, X } from "@phosphor-icons/react";
...
type Tab = "appearance" | "connectors" | "memory" | "notifications" | "usage";
type Props = { productName: string; open: boolean; initialTab?: Tab; runtimeStatus: RuntimeStatus; onSettingsSnapshot?: (snapshot: SettingsSnapshot) => void; onOpenMemory?: () => void; onClose: () => void };
...
export function CustomizationPanel({ productName, open, initialTab = "appearance", runtimeStatus, onSettingsSnapshot, onOpenMemory, onClose }: Props) {
...
function Connectors({ settings, onChanged }: { settings: SettingsSnapshot | null; onChanged: () => void }) {
  ...
  const disconnect = async (url: string) => { setBusy(true); setError(null); try { const response = await fetch(url, { method: "POST" }); const body: unknown = await response.json().catch(() => null); if (!response.ok) throw new Error(body && typeof body === "object" && "error" in body && typeof body.error === "string" ? body.error : "No se ha podido desconectar."); onChanged(); } catch (cause) { setError(cause instanceof Error ? cause.message : "No se ha podido desconectar."); } finally { setBusy(false); } };
  return <section><SectionTitle>Conectores personales</SectionTitle><p className="mb-4 text-[12px] leading-5 text-[var(--text-muted)]">Cada conexión pertenece únicamente a tu usuario. Nunca se comparte una cuenta personal con otro empleado.</p><div className="space-y-3">{settings?.connectors.map((connector) => <article key={connector.id} className="rounded-[16px] border border-[var(--border)] bg-[var(--surface)] p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[12px] font-semibold">{connector.label}</p><p className="mt-1 text-[11px] text-[var(--text-subtle)]">{connector.accountEmail ?? connector.statusDetail}</p></div><span className={`rounded-full px-2 py-1 text-[9px] font-semibold ${connector.status === "connected" ? "bg-[var(--positive-soft)] text-[var(--positive)]" : "bg-[var(--surface-muted)] text-[var(--text-subtle)]"}`}>{connector.status === "connected" ? "Conectado" : connector.status === "requires_login" ? "Sin conectar" : "No disponible"}</span></div><p className="mt-3 text-[10px] leading-4 text-[var(--text-subtle)]">Permiso: lectura de Gmail. No permite enviar, borrar ni modificar mensajes.</p><div className="mt-4 flex justify-end">{connector.status === "connected" && connector.disconnectUrl ? <button type="button" disabled={busy} onClick={() => void disconnect(connector.disconnectUrl!)} className="rounded-lg border border-[var(--border)] px-3 py-2 text-[11px] font-semibold disabled:opacity-50">Desconectar</button> : connector.connectUrl ? <a href={connector.connectUrl} className="rounded-lg bg-[var(--brain-accent)] px-3 py-2 text-[11px] font-semibold text-[var(--brain-contrast)]">Conectar Gmail</a> : null}</div></article>)}{settings && settings.connectors.length === 0 ? <p className="text-[11px] text-[var(--text-subtle)]">No tienes conectores autorizados.</p> : null}</div>{error ? <p role="alert" className="mt-3 text-[11px] text-[var(--danger)]">{error}</p> : null}</section>;
}
function MemorySettings({ settings, onOpenMemory }: { settings: SettingsSnapshot | null; onOpenMemory?: () => void }) { return <section><SectionTitle>Memoria de trabajo</SectionTitle><div className="rounded-[16px] border border-[var(--border)] bg-[var(--surface)] p-4"><p className="text-[12px] font-semibold">Activa en tu entorno privado</p><p className="mt-2 text-[11px] leading-5 text-[var(--text-subtle)]">Las sugerencias no se guardan solas. Puedes confirmar, editar, rechazar o eliminar cada memoria con su alcance y procedencia visibles.</p><ul className="mt-3 space-y-1 text-[10px] text-[var(--text-subtle)]"><li>Alcances: privada, proyecto y empresa con permiso administrativo.</li><li>Aislamiento: memoria y runtime separados por empleado.</li><li>El historial compartido del ordenador no se usa como memoria.</li></ul>{settings?.memory.enabled && onOpenMemory ? <button type="button" className="mt-4 rounded-lg bg-[var(--brain-accent)] px-3 py-2 text-[11px] font-semibold text-[var(--brain-contrast)]" onClick={onOpenMemory}>Gestionar memoria</button> : null}</div></section>; }
