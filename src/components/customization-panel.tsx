...
function connectorCallbackFailed() {
  const params = new URLSearchParams(window.location.search);
  return params.get("connection") === "failed" || [params.get("gmail"), params.get("outlook")].some(status => status === "failed" || status === "denied");
}
function Connectors({ settings, onChanged }: { settings: SettingsSnapshot | null; onChanged: () => void }) {
  const callbackFailed = useSyncExternalStore(subscribeConnectorCallback, connectorCallbackFailed, () => false);
