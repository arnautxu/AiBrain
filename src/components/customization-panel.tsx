...
function Connectors({ settings, onChanged }: { settings: SettingsSnapshot | null; onChanged: () => void }) {
  ...
  const row = (connector: NonNullable<SettingsSnapshot>["connectors"][number]) => <article key={connector.id} className="flex min-h-20 items-center gap-3 border-b border-[var(--border-subtle)] px-3 py-3 last:border-0">
    ...
    <div className="flex shrink-0 flex-col items-end gap-2">
      {connector.status === "connected" ? <span className="text-[11px] font-medium text-[var(--positive)]">Conectado</span> : connector.connectUrl ? <a aria-label={`Conectar ${connector.label}`} href={connector.connectUrl} className="rounded-full bg-[var(--brain-accent)] px-4 py-2 text-[11px] font-semibold text-[var(--brain-contrast)]">Conectar</a> : <span className="text-[11px] text-[var(--text-subtle)]">{connector.status === "admin_setup_required" ? "Falta configuración" : "No disponible"}</span>}
      {connector.disconnectUrl ? <button type="button" disabled={busy} aria-label={`Desconectar ${connector.label}`} onClick={() => void disconnect(connector.disconnectUrl!)} className="text-[10px] text-[var(--text-subtle)] underline underline-offset-2 disabled:opacity-50">Desconectar</button> : null}
      {connector.status === "connected" && connector.connectUrl ? <a href={connector.connectUrl} aria-label={`Reconectar ${connector.label}`} className="text-[10px] text-[var(--text-subtle)]">Reconectar</a> : null}
    </div>
  </article>;
  return <section><SectionTitle>Conectores personales</SectionTitle>
    {callbackStatus === "failed" ? <p role="alert" className="mb-3 text-xs text-[var(--danger)]">No se completó la conexión. Vuelve a intentarlo.</p> : callbackStatus === "returned" ? <p role="status" className="mb-3 text-xs text-[var(--text-subtle)]">Consulta abajo el estado verificado de tus conexiones.</p> : null}
    <p className="mb-4 text-[12px] leading-5 text-[var(--text-muted)]">Conecta tus apps para usarlas con @ en tus conversaciones. Cada cuenta pertenece solo a tu usuario.</p>
    <input type="search" aria-label="Buscar apps" placeholder="Buscar apps…" value={query} onChange={event => setQuery(event.target.value)} className="mb-5 min-h-10 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-[16px] outline-none focus:border-[var(--focus)] md:text-[13px]" />
    {connected.length ? <><h4 className="mb-2 text-[12px] font-semibold">Conectadas</h4><div className="mb-5 overflow-hidden rounded-xl border border-[var(--border)]">{connected.map(row)}</div></> : null}
    {available.length ? <><h4 className="mb-2 text-[12px] font-semibold">{query ? "Apps disponibles" : "Sugeridas para tu empresa"}</h4><div className="overflow-hidden rounded-xl border border-[var(--border)]">{available.map(row)}</div></> : null}
    {settings && !matches.length ? <p className="text-[12px] text-[var(--text-subtle)]">{query ? "No hay apps autorizadas que coincidan." : "No hay conexiones habilitadas para tu usuario."}</p> : null}
    <p className="mt-4 text-[11px] leading-4 text-[var(--text-subtle)]">Catálogo autorizado para tu empresa y tus grupos. Acceso limitado a las herramientas de lectura revisadas.</p>
    {error ? <p role="alert" className="mt-3 text-[11px] text-[var(--danger)]">{error}</p> : null}
  </section>;
}
