...
function parseMcpInventory(value: unknown) {
  ...
  return (value as { data: unknown[] }).data.flatMap((candidate) => {
    ...
    const record = candidate as { name?: unknown; tools?: unknown };
    if (typeof record.name !== "string" || !record.tools || typeof record.tools !== "object" || Array.isArray(record.tools)) return [];
    return [{ name: record.name, tools: Object.keys(record.tools as Record<string, unknown>) }];
  });
}

/** Server-only production assembly; the core reports only sanitized status. */
export function codexManagedAppPreflightDependencies(): CodexManagedAppPreflightDependencies {
