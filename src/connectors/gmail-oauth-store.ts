...
import type { GmailTokenSet } from "@/connectors/gmail-contracts";
import { atomicWriteFile, ResourceLockManager } from "@/storage";

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/u;
const HASH = /^[0-9a-f]{64}$/u;
const BASE64 = /^[A-Za-z0-9+/]+={0,2}$/u;
const CREDENTIAL_REF = /^gmail:[0-9a-f-]{36}$/u;
...
export class GmailOAuthStoreError extends Error {
  constructor(readonly code: string, message: string) { super(message); this.name = "GmailOAuthStoreError"; }
}
...
function parseJson(filePath: string, raw: string) {
  try { return JSON.parse(raw) as unknown; }
  catch { throw new GmailOAuthStoreError("GMAIL_OAUTH_STATE_CORRUPT", `Invalid Gmail OAuth state at ${filePath}.`); }
}
...
function parseState(value: unknown): OAuthStateRecord {
  ...
  if (!record(value) || !exactKeys(value, keys) || value.schemaVersion !== 1 || typeof value.stateHash !== "string" || !HASH.test(value.stateHash) ||
    ...
    throw new GmailOAuthStoreError("GMAIL_OAUTH_STATE_CORRUPT", "Gmail OAuth state is invalid.");
  }
...
function parseTokenRecord(value: unknown): EncryptedTokenRecord {
  ...
  if (!record(value) || !exactKeys(value, keys) || value.schemaVersion !== 1 || typeof value.installationId !== "string" ||
    ...
    throw new GmailOAuthStoreError("GMAIL_TOKEN_CORRUPT", "Encrypted Gmail token record is invalid.");
  }
  return value as EncryptedTokenRecord;
}

function parseTokenSet(value: unknown): GmailTokenSet {
  if (!record(value) || !exactKeys(value, ["accessToken", "refreshToken", "expiresAt", "scopes", "tokenType"]) ||
    ...
    throw new GmailOAuthStoreError("GMAIL_TOKEN_CORRUPT", "Decrypted Gmail token payload is invalid.");
  }
...
async function privateDirectory(directory: string) {
  ...
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) throw new GmailOAuthStoreError("GMAIL_OAUTH_PATH_UNSAFE", "Gmail OAuth storage must be a real directory.");
  await chmod(directory, 0o700);
}

export function gmailOAuthEncryptionKey(encoded: string | undefined) {
  if (!encoded) throw new GmailOAuthStoreError("GMAIL_OAUTH_ENCRYPTION_KEY_MISSING", "Gmail OAuth encryption key is not configured.");
  const key = Buffer.from(encoded, "base64");
  if (key.length !== 32 || key.toString("base64").replace(/=+$/u, "") !== encoded.replace(/=+$/u, "")) {
    throw new GmailOAuthStoreError("GMAIL_OAUTH_ENCRYPTION_KEY_INVALID", "Gmail OAuth encryption key must be 32 random base64 bytes.");
  }
  return key;
}
...
export class FileGmailOAuthStateStore {
  ...
  async create(userId: string, redirectUri: string) {
    if (!UUID.test(userId)) throw new GmailOAuthStoreError("GMAIL_OAUTH_USER_INVALID", "OAuth user is invalid.");
  ...
  async consume(userId: string, rawState: string) {
    if (!UUID.test(userId) || !/^[A-Za-z0-9_-]{40,100}$/u.test(rawState)) throw new GmailOAuthStoreError("GMAIL_OAUTH_STATE_INVALID", "OAuth state is invalid.");
    const stateHash = createHash("sha256").update(rawState).digest("hex");
    return this.locks.withLock(`gmail-oauth-state:${stateHash}`, async () => {
      const filePath = this.statePath(stateHash);
      const value = parseState(parseJson(filePath, await readFile(filePath, "utf8")));
      if (value.installationId !== this.config.installationId || value.userId !== userId) throw new GmailOAuthStoreError("GMAIL_OAUTH_STATE_IDENTITY_MISMATCH", "OAuth state belongs to another authenticated user.");
      if (value.consumedAt || !value.codeVerifier) throw new GmailOAuthStoreError("GMAIL_OAUTH_STATE_REPLAYED", "OAuth state has already been consumed.");
      if (Date.parse(value.expiresAt) <= this.now()) throw new GmailOAuthStoreError("GMAIL_OAUTH_STATE_EXPIRED", "OAuth state has expired.");
...
export class FileGmailTokenStore {
  private readonly locks: ResourceLockManager;
  constructor(private readonly config: Readonly<InstallationConfig>, private readonly key: Buffer, private readonly now: () => number = Date.now) {
    if (key.length !== 32) throw new GmailOAuthStoreError("GMAIL_OAUTH_ENCRYPTION_KEY_INVALID", "Gmail OAuth encryption key must be 32 bytes.");
    this.locks = new ResourceLockManager({ rootDirectory: path.join(path.resolve(config.paths.dataRoot), "connectors", "gmail", "token-locks") });
  }
  private async tokenPath(userId: string) {
    ...
    if (!rootMeta.isDirectory() || rootMeta.isSymbolicLink() || !userMeta.isDirectory() || userMeta.isSymbolicLink() || (userMeta.mode & 0o077) !== 0 || !inside(await realpath(usersRoot), await realpath(userRoot))) {
      throw new GmailOAuthStoreError("GMAIL_OAUTH_PATH_UNSAFE", "Gmail token user root is unsafe.");
    }
    const root = path.join(userRoot, "connectors", "gmail");
    await privateDirectory(root);
    if (!inside(await realpath(userRoot), await realpath(root))) throw new GmailOAuthStoreError("GMAIL_OAUTH_PATH_UNSAFE", "Gmail token storage escapes the user root.");
    return path.join(root, "oauth-token.json");
  }
  private encrypt(userId: string, credentialRef: string, token: GmailTokenSet) {
  ...
  async put(userId: string, token: GmailTokenSet, credentialRef?: string) {
    const filePath = await this.tokenPath(userId);
    return this.locks.withLock(`gmail-token:${this.config.installationId}:${userId}`, async () => {
      ...
      if (current && current.userId !== userId) throw new GmailOAuthStoreError("GMAIL_TOKEN_IDENTITY_MISMATCH", "Gmail token belongs to another user.");
      const reference = credentialRef ?? current?.credentialRef ?? `gmail:${randomUUID()}`;
  ...
  async read(userId: string, credentialRef: string) {
    const filePath = await this.tokenPath(userId);
    const record = parseTokenRecord(parseJson(filePath, await readFile(filePath, "utf8")));
    if (record.installationId !== this.config.installationId || record.userId !== userId || record.credentialRef !== credentialRef) {
      throw new GmailOAuthStoreError("GMAIL_TOKEN_IDENTITY_MISMATCH", "Gmail token belongs to another identity.");
    }
    try { return { token: this.decrypt(record), tokenVersion: record.version }; }
    catch { throw new GmailOAuthStoreError("GMAIL_TOKEN_DECRYPT_FAILED", "Gmail token could not be decrypted."); }
  }
