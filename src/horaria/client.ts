...
export async function loadHorariaConfig(installation: Readonly<InstallationConfig>): Promise<HorariaConfig> {
  ...
  try {
    ...
    if (!stat.isFile() || stat.size > 64_000 || (stat.mode & 0o077)) throw new Error("La configuració d’horaris ha de ser privada.");
    ...
    if (c.installationId !== installation.installationId || typeof c.secret !== "string" || c.secret.length < 32 ||
      !["http:", "https:"].includes(url.protocol) || url.username || url.password || url.pathname !== "/" || url.search || url.hash || typeof c.eventsEnabled !== "boolean" || !c.users || typeof c.users !== "object" || Array.isArray(c.users)) throw new Error("Configuració d’horaris invàlida.");
...
export async function callHoraria(config: HorariaConfig, session: AuthSession, input: OperationInput, projectWorkspace: string) {
  if (session.provider !== "local" || session.tenant.id !== config.installationId || !Object.hasOwn(config.users, session.user.id)) throw new Error("No tens accés als horaris d’aquesta instal·lació.");
  ...
  if (input.uploadPath) {
    if (path.isAbsolute(input.uploadPath) || input.uploadPath.split(/[\\/]/).includes("..")) throw new Error("La imatge ha de pertànyer al projecte actual.");
    ...
    if (!mime) throw new Error("Adjunta una foto PNG o JPEG.");
  ...
  const response = await fetch(new URL(op.target, config.baseUrl), { method: op.method, headers: { "x-aibrain-authorization": token, ...(contentType ? { "content-type": contentType } : {}) }, body: body.length ? body : undefined, redirect: "error", cache: "no-store", signal: AbortSignal.timeout(op.effect === "ai" ? 20 * 60_000 : 90_000) });
