...
const worker = path.join(process.cwd(), "tests", "fixtures", "document-storage-worker.ts");
...
function start(
  ...
  const child = spawn(
    tsx,
    [worker, lockRoot, capacityRoot, userId, mode, String(holdMs), String(maximum)],
    {
      cwd: process.cwd(),
...
describe("document storage admission across real processes", () => {
  it("shares slots across users, rejects overflow and recovers a crashed owner", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "aibrain-document-storage-process-"));
    roots.push(root);
    const capacityRoot = path.join(root, "data");
    const lockRoot = path.join(root, "locks");
    await mkdir(capacityRoot, { mode: 0o700 });
    const first = start(lockRoot, capacityRoot, "user-a", "crash", 0);
    const second = start(lockRoot, capacityRoot, "user-b", "crash", 0);
    ...
    const overflow = start(lockRoot, capacityRoot, "user-c", "hold", 0);
    ...
    const recovered = start(lockRoot, capacityRoot, "user-c", "hold", 0);
