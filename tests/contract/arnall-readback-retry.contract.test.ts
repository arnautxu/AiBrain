...
async function firstCollectionFixture(): Promise<string> {
  ...
  const artifacts = new Map<string, string>([
    ["release-ci-readback.json", document({ schemaVersion: 1, kind: "aibrain-release-ci-readback", source: "ci", candidateSha: revision, ciSha: revision, capturedAt, provenance: { kind: "file", sha256: "d".repeat(64) } })],
  ...
  await writeFile(path.join(root, "backend-ci-source.json"), document({ schemaVersion: 1, workflow: "Backend CI", conclusion: "success", headSha: revision, runId }), { mode: 0o600 });
  const manifest = {
    schemaVersion: 1, releaseSha: revision, ciRunId: runId, capturedAt,
    evidence: [
...
async function retryValidator(): Promise<string> {
  ...
  await writeFile(pathname, `${sourceable}\nfail() { return 1; }\nrequire_root_owned_file() { [[ -f "$1" && ! -L "$1" ]]; }\nrequire_root_owned_directory() { [[ -d "$1" && ! -L "$1" ]]; }\nvalidate_existing_release_readbacks "$1" "$2" "$3"\n`, { mode: 0o700 });
  await chmod(pathname, 0o700);
  return pathname;
}

function validate(script: string, root: string, requestedRunId = runId): void {
  execFileSync("bash", [script, revision, requestedRunId, root], { stdio: "pipe" });
}
...
describe("Arnall release-readback retry contract", () => {
  it("accepts the exact package from a first collection, then rejects a different run ID", async () => {
    ...
    expect(() => validate(script, root, "33165012870")).toThrow();
  });
