...
test("dark shell, guided actions, turn capabilities and Review have no blocking axe violations", async ({ page }) => {
  ...
  const events = [
    ...
    { type: "approval", item: { id: "approval-dark", kind: "command", title: "Ejecutar comprobación", detail: "Comprueba únicamente el estado sintético.", command: "check --synthetic", status: "pending" } },
